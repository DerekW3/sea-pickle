...
def extract_tuple(chunks: list[bytes], idx: int) -> bytes:
    """extracts a 1, 2 or 3 element tuple from a list
        of chunks.

    Args:
        chunks (list[bytes]): list of chunks generated by get chunks.
        idx (int): the start idx of the tuple (that which contains
            the TUPLE1, TUPLE2 or TUPLE3 opcode)

    Returns:
        bytes: the tuple object ending in the chunk at idx
    """
    num_remains = 1
    curr_idx = idx
    res = b""
...
def extract_sequence(chunks: list[bytes], idx: int) -> bytes:
    """extracts an arbitrary sized sequence from a list of
        chunks. This includes list, dictionaries and tuples
        longer than 3 elements.

    Args:
        chunks (list[bytes]): the chunks generated by get_chunks
        idx (int): the start index of the sequence.

    Returns:
        bytes: the extracted sequence object encoded.
    """
    num_remains = 1
    ident = chunks[idx][:1]
    curr_idx = idx
